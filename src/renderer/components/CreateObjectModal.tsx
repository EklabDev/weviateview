...
import { createObject, initializeWeaviate } from '../lib/weaviate';
...
export function CreateObjectModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await createObject(collectionName, formData);
      onSuccess();
      onClose();
    } catch (err) {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 text-gray-900">
      <div className="bg-white rounded-lg p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        <h2 className="text-2xl font-bold mb-4">Create New Object in {collectionName}</h2>
